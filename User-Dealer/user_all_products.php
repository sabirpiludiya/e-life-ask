...
session_start();
	...
	// To pagination 
	$select_page = mysql_query("Select * from Tbl_product p, Tbl_follow_block f where p.dealer_id = f.dealer_id and f.user_id = $_SESSION[user_id] and block_status = 0 and product_status=1");
...
{
	$query = mysql_query("select * from Tbl_product p, Tbl_follow_block f where p.dealer_id = f.dealer_id and f.user_id = $_SESSION[user_id] and block_status = 0 and product_status=1 order by product_name".$limit);
}
else if(isset($_POST['btn_desc']))
{
	$query = mysql_query("select * from Tbl_product p, Tbl_follow_block f where p.dealer_id = f.dealer_id and f.user_id = $_SESSION[user_id] and block_status = 0 and product_status=1 order by product_name desc".$limit);
}
else if(isset($_POST['btn_search']) && $_POST['ddl_prosearch'] == "Title")
{
	$query = mysql_query("select * from Tbl_product p, Tbl_follow_block f where p.dealer_id = f.dealer_id and f.user_id = $_SESSION[user_id] and block_status = 0 and product_status=1 and product_name like '%$_POST[txt_search]%'".$limit);
}
else if(isset($_POST['btn_search']) && $_POST['ddl_prosearch'] == "Brand")
{
	$query = mysql_query("select * from Tbl_product p, Tbl_follow_block f where p.dealer_id = f.dealer_id and f.user_id = $_SESSION[user_id] and block_status = 0 and product_status=1 and product_brand like '%$_POST[txt_search]%'".$limit);
}
else if(isset($_POST['btn_search']) && $_POST['ddl_prosearch'] == "Model")
{
	$query = mysql_query("select * from Tbl_product p, Tbl_follow_block f where p.dealer_id = f.dealer_id and f.user_id = $_SESSION[user_id] and block_status = 0 and product_status=1 and product_model_no like '%$_POST[txt_search]%'".$limit);
}
else
{
	$query = mysql_query("select * from Tbl_product p, Tbl_follow_block f where p.dealer_id = f.dealer_id and f.user_id = $_SESSION[user_id] and block_status = 0 and product_status=1".$limit);
}
?>
...
<b>Product Description :</b> <br>	<?php echo $row['product_description']; ?> <br>  <hr style="border:dashed 1px"><h3 style="color:#9966FF">Comments</h3><?php $selectc = "select * from Tbl_comment where product_id = '$row[product_id]' and comment_status = 1 order by comment_date_time DESC limit 3";
...
<b style="font-size:20px"><a href="user_view_comments.php?product=<?php echo $row['product_id'];?>">Post & View All Comments...</a></b> <?php  }
		...
		<?php	}
			  ...
			  <center>
			  <a href="user_all_products.php?page=1"><img src="images/first.png" height="5%" width="5%"></a> |
			  <a href="user_all_products.php?page=<?php echo $pre_page; ?>"><img src="images/prev.png" height="4%" width="4%"></a> 
			  <a href="user_all_products.php?page=<?php echo $next_page; ?>"><img src="images/next.png" height="4%" width="4%"></a> |
			  <a href="user_all_products.php?page=<?php echo $total_page; ?>"><img src="images/last.png" height="5%" width="5%"></a>
			  </center>
